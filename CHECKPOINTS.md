...
- Per member:
    ...
    - 2026-09-25 - 2453210 Phan The Thong: Completed W03-T3 Sets, Parameters, and Decision Variables specification and implementation for Module 2 ILP. Formally defined sets $I$ (|I|=73), $J$ (|J|=65), $C$ (|C|=2); defined parameters demand ($d_j$, total 769 slots), availability ($a_{ij}$), overlap ($o_{jk}$), and campus/location preferences ($p_{ij}$ with weight 1.28). Defined decision variables $x_{ij} \in \{0, 1\}$ (Binary, 4,745 variables), workload $w_i \in \mathbb{Z}_{\ge 0}$ (Integer, 73 variables), and minimax maximum load $t \ge 0$ (Continuous/Integer, 1 variable). Wrote comprehensive mathematical specification in `m2_ilp/model_spec.md` and implemented typed variable factory, validation, and PuLP integration in `m2_ilp/variables.py`. (Req 2.1, 2.2, W03-T3)
