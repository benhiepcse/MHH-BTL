...
# ============================================================
# W03-T1 configuration
# ============================================================

EXPECTED_ROWS = 769          # data rows, excluding the header
EXPECTED_COLS = 9
...
try:
    from m1_logic.logic_to_lp import LogicToLPBridge  # type: ignore
except ImportError as exc:
    BRIDGE_IMPORT_ERROR = str(exc)
    try:
        from logic_to_lp import LogicToLPBridge  # type: ignore
...
def parse_start_time(value: Any):
    """Parse Vietnamese time such as 18g15 or fallback format 18:15."""
    if pd.isna(value):
        return None
...
def extract_shift_number(text: Any) -> int | None:
    """Extract Ca N from text such as 'Ca 5: 18g15-20g45'."""
    if pd.isna(text):
        return None
...
# ============================================================
# 1. Read dataset + schema validation
# ============================================================
...
def data_quality_report(df: pd.DataFrame) -> dict[str, Any]:
    ...
    # For the assignment relation, count duplicate rows beyond the first
    # occurrence of the same (invigilator, session) pair.
...
def extract_sets(df: pd.DataFrame) -> dict[str, list[Any]]:
    return {
        ...
        # Keep original text values for the schema report
        "start_times": sorted(
...
# ============================================================
# 3. Baseline assignment relation Assign(i,j)
# ============================================================
...
# ============================================================
# 4. Session-level analysis + meaning of MS Ca thi
# ============================================================

def build_session_table(df: pd.DataFrame) -> pd.DataFrame:
    """
    Collapse assignment records into one row per MS Ca thi for semantic/time analysis.

    Important:
    - MS Ca thi is the session/shift identifier.
    - campus/task are retained as sets because the same session code may occur
      on multiple assignment rows.
    - end_time is DERIVED from start time + duration, not read from a raw column.
    """
...
def generate_logic_to_lp_outputs(
    ...
    """
    Apply the team's Req 1.3 bridge to the real-data overlap relation:

        Overlap(j,k) -> not(Assign(i,j) and Assign(i,k))
        -> x_ij + x_ik <= 1

    The dataset does not expose explicit Busy(i,j) or required-capacity fields,
    so those are reported as unavailable rather than fabricated.
    """
    result: dict[str, Any] = {
...
def analyze_dataset(
    ...
    # W03-T1 explicitly requires 769 data rows.
